...
import { reqGetQCode } from "@api/pay";

export default {
  name: "Pay",
  methods: {
    async submit() {
      const result = await reqGetQCode(this.$route.query.orderId);
      // console.log(result);
      // 转换二维码
      QRCode.toDataURL(result.codeUrl)
        .then((url) => {
          this.$alert(
            ...
            "请使用微信扫码支付",
            {
              showCancelButton: true, // 显示两个按钮
              confirmButtonText: "我已成功支付",
              cancelButtonText: "支付过程中遇到问题",
              type: "warning",
              center: true, // 居中
              showClose: false, // 是否取消右上角取消按钮
              dangerouslyUseHTMLString: true, // 解析html 片段
            }
          )
            // 点击了成功按钮
            .then(() => {
        ...
        .catch(() => {
          this.$message.error('支付中遇到问题，请重新试试')
        });
    },
  },
};
